...
function validateScale03(scale03) {
  const expectedSummary = { planned: 24, complete: 23, pending: 1, changed: 19, alreadyCompliant: 4 }
  ...
  if (complete.length !== 23 || pending.length !== 1) fail('SCALE-03 stale status counts')
  if (complete.filter(entry => entry.disposition === 'changed').length !== 19 || complete.filter(entry => entry.disposition === 'already-compliant').length !== 4) {
