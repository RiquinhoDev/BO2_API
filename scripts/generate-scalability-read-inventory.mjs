...
function validateScale03(scale03) {
  const expectedSummary = { planned: 24, complete: 21, pending: 3, changed: 17, alreadyCompliant: 4 }
  ...
  if (complete.length !== 21 || pending.length !== 3) fail('SCALE-03 stale status counts')
  if (complete.filter(entry => entry.disposition === 'changed').length !== 17 || complete.filter(entry => entry.disposition === 'already-compliant').length !== 4) {
