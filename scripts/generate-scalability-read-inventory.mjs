...
function validateScale03(scale03) {
  const expectedSummary = { planned: 24, complete: 22, pending: 2, changed: 18, alreadyCompliant: 4 }
  ...
  if (complete.length !== 22 || pending.length !== 2) fail('SCALE-03 stale status counts')
  if (complete.filter(entry => entry.disposition === 'changed').length !== 18 || complete.filter(entry => entry.disposition === 'already-compliant').length !== 4) {
