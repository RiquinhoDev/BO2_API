...
function validateScale03(scale03) {
  const expectedSummary = { planned: 24, complete: 20, pending: 4, changed: 16, alreadyCompliant: 4 }
  ...
  if (complete.length !== 20 || pending.length !== 4) fail('SCALE-03 stale status counts')
  if (complete.filter(entry => entry.disposition === 'changed').length !== 16 || complete.filter(entry => entry.disposition === 'already-compliant').length !== 4) {
