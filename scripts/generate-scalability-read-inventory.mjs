...
function validateScale03(scale03) {
  const expectedSummary = { planned: 24, complete: 19, pending: 5, changed: 15, alreadyCompliant: 4 }
  ...
  if (complete.length !== 19 || pending.length !== 5) fail('SCALE-03 stale status counts')
  if (complete.filter(entry => entry.disposition === 'changed').length !== 15 || complete.filter(entry => entry.disposition === 'already-compliant').length !== 4) {
