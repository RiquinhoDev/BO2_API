// ════════════════════════════════════════════════════════════
// TESTE: Endpoint /api/cron/daily-pipeline (Pipeline Completo)
...
async function main() {
  ...
  console.log('TESTE: Endpoint /api/cron/daily-pipeline (Pipeline Completo)')
  console.log('━'.repeat(70))
  console.log('')
  ...
  // ════════════════════════════════════════════════════════════
  console.log('═'.repeat(70))
  console.log('FASE 1: Capturar estado ANTES do pipeline')
  console.log('═'.repeat(70))
  ...
  const validUserProducts = allUserProducts.filter(up => {
    if (!up.userId?.email) return false
    const productIdStr = up.productId?._id?.toString()
    if (discordProductIds.has(productIdStr)) return false
    return true
  })

  console.log(`Total UserProducts a verificar: ${validUserProducts.length}`)

  // Capturar estado ANTES para cada user
  type UserState = {
    email: string
    userId: string
    productId: string
  ...
  // ════════════════════════════════════════════════════════════
  // FASE 2: Chamar endpoint do pipeline completo
  // ════════════════════════════════════════════════════════════
  console.log('\n' + '═'.repeat(70))
  console.log('FASE 2: Chamar endpoint /api/cron/daily-pipeline')
  console.log('═'.repeat(70))

  const endpointUrl = `${API_URL}/api/cron/daily-pipeline`
  console.log(`\nURL: ${endpointUrl}`)
  console.log('A chamar endpoint... (PODE DEMORAR VÁRIAS HORAS!)\n')
  console.log('⚠️  Se quiser cancelar, pressione Ctrl+C\n')
  ...
  try {
    ...
    console.log(`✅ Pipeline completou em ${Math.floor(endpointDuration / 60)}min ${endpointDuration % 60}s`)
    console.log(`   Success: ${endpointResult.success}`)

    if (endpointResult.steps) {
      console.log('\n   📊 Resumo dos steps:')
      console.log(`      Step 1 - Hotmart:    ${endpointResult.steps.syncHotmart?.duration || 0}s`)
      console.log(`      Step 2 - CursEduca:  ${endpointResult.steps.syncCursEduca?.duration || 0}s`)
      console.log(`      Step 3 - Pre-create: ${endpointResult.steps.preCreateTags?.duration || 0}s`)
      console.log(`      Step 4 - Engagement: ${endpointResult.steps.recalcEngagement?.duration || 0}s`)
      console.log(`      Step 5 - Tag Rules:  ${endpointResult.steps.evaluateTagRules?.duration || 0}s`)
    }

    if (endpointResult.summary) {
      console.log('\n   📈 Resumo:')
      console.log(`      Tags aplicadas: ${endpointResult.summary.tagsApplied || 0}`)
      console.log(`      Tags removidas: ${endpointResult.steps?.evaluateTagRules?.stats?.tagsRemoved || 0}`)
    }

    if (endpointResult.errors?.length > 0) {
      console.log(`\n   ❌ Erros: ${endpointResult.errors.length}`)
    }
  } catch (err: any) {
  ...
  for (const state of userStates) {
    try {
      // Tags actuais na AC (DEPOIS do pipeline)
  ...
  const reportPath = path.join(reportDir, `full-pipeline-test-${timestamp}.json`)
