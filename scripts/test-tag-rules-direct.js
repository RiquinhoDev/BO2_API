...
async function main() {
    ...
    try {
        ...
        const { executeTagRulesOnly } = await Promise.resolve().then(() => __importStar(require('../src/services/cron/dailyPipeline.service')));
    ...
    const reportPath = path_1.default.join(reportDir, `tag-rules-direct-${timestamp}.json`);
