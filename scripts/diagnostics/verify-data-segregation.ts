...
async function verifyDataSegregation() {
  try {
    ...
    for (const user of users) {
      ...
      if (user.discord?.userId) platforms.push('discord');
      if (user.hotmart?.email) platforms.push('hotmart');
      if (user.curseduca?.email) platforms.push('curseduca');
      ...
      if (platforms.length > 1) {
        ...
        const missingInConsolidated = uniquePlatformCourses.filter(
          course => !consolidated.includes(course)
        );
        ...
        const missingPlatforms = expectedPlatforms.filter(
          p => !user.allPlatforms?.includes(p as any)
        );
    ...
    const userProducts = await UserProduct.find({}).populate('userId productId');
