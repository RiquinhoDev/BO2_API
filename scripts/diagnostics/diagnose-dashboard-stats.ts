...
async function diagnoseDashboardStats() {
  try {
    ...
    const multiPlatform = await User.countDocuments({
      $expr: { $gt: [{ $size: { $ifNull: ['$allPlatforms', []] } }, 1] }
    });
    console.log(`Multi-platform Users: ${multiPlatform}`);
    
    // Breakdown detalhado
    const platformCombinations = await User.aggregate([
      {
        $match: {
          $expr: { $gt: [{ $size: { $ifNull: ['$allPlatforms', []] } }, 1] }
        }
    ...
    const ogiUsers = await User.countDocuments({
      consolidatedCourses: 'ogi'
    });
    console.log(`OGI Students: ${ogiUsers}`);
    
    const clarezaUsers = await User.countDocuments({
    ...
    if (totalUserProducts > 0) {
      const upByProduct = await UserProduct.aggregate([
        {
          $lookup: {
            from: 'products',
    ...
    const ogiProduct = await Product.findOne({ code: 'OGI-V1' });
    const ogiUserProducts = ogiProduct 
      ? await UserProduct.countDocuments({ productId: ogiProduct._id })
      : 0;
    ...
    for (const user of sampleUsers) {
      ...
      const ups = await UserProduct.find({ userId: user._id }).populate('productId');
      console.log(`  UserProducts: ${ups.length}`);
      ups.forEach(up => {
        console.log(`    - ${(up.productId as any).code}: ${up.platformData?.platformId || 'N/A'}`);
      });
    }
