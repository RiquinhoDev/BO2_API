...
function build(
  over: {
    item?: Partial<UniversalSourceItem>
  ...
  const resolvedClass =
    over.resolvedClass ?? (item.classId ? { classId: item.classId, className: item.className || `Turma ${item.classId}` } : undefined)
...
describe('buildHotmartMutationPlan — classes', () => {
  it('sets a new class, root ids and combined for an item with classId', () => {
    const plan = build({ item: { classId: 'C1', className: 'Item Name' }, resolvedClass: { classId: 'C1', className: 'Nome Real' } })
  ...
  it('preserves an existing active class when the item has no classId', () => {
    const user: HotmartUserState = { classId: 'X', hotmart: { enrolledClasses: [{ classId: 'ACT', className: 'Ativa', isActive: true }] } }
  ...
  it('keeps curseduca classes in combined.allClasses on a hotmart plan', () => {
    ...
    const plan = build({ item: { classId: 'C1' }, resolvedClass: { classId: 'C1', className: 'C1 Nome' }, user })
...
describe('buildHotmartMutationPlan — fields', () => {
  ...
  it('is deterministic under a fixed clock for identical input', () => {
    const a = build({ item: { classId: 'C1' }, resolvedClass: { classId: 'C1', className: 'C1' } })
    const b = build({ item: { classId: 'C1' }, resolvedClass: { classId: 'C1', className: 'C1' } })
    expect(a).toEqual(b)
  })
})
...
describe('hotmartPlanToUpdateFields', () => {
  it('flattens to the exact dotted mongo paths', () => {
    const plan = build({ item: { classId: 'C1', hotmartUserId: 'h-9', accessCount: 3 }, resolvedClass: { classId: 'C1', className: 'C1 Nome' } })
