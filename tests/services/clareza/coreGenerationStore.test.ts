...
describe('MongooseCoreGenerationStore', () => {
  it('refuses a candidate write when the outer receipt lost ownership', async () => {
    ...
    }, () => store.createCandidate(candidate('generation-fenced', '2026-09-01T01:00:00.000Z'))))
      .rejects.toThrow('ownership lost')

    await expect(ClarezaCoreGeneration.countDocuments({ generationId: 'generation-fenced' }))
      .resolves.toBe(0)
  })
