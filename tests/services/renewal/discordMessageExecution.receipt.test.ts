...
test('same request replay returns the canonical result without publishing twice', async () => {
  const first = await sendWithRequestId(message, 'message-a')
  const replay = await sendWithRequestId(message, 'message-a')

  expect(replay).toEqual(first)
  expect(replay.success).toBe(true)
  expect(mockAxiosPost).toHaveBeenCalledTimes(1)
  expect(await DiscordMessageLog.countDocuments({})).toBe(1)
})

test('replay wins over a later switch or default-channel change', async () => {
  const first = await sendWithRequestId({ ...message, channelId: undefined }, 'message-a')
  mockRuntimeConfig.renewal.discordMessagesEnabled = false
  mockRuntimeConfig.renewal.discordMessageChannelId = 'channel-2'
  mockRuntimeConfig.renewal.discordMessageChannels = ['channel-2:Other']

  const replay = await sendWithRequestId({ ...message, channelId: undefined }, 'message-a')

  expect(replay).toEqual(first)
  expect(mockAxiosPost).toHaveBeenCalledTimes(1)
})

test('same request ID with a different payload is rejected atomically', async () => {
  await sendWithRequestId(message, 'message-a')
  const reused = await sendWithRequestId({ ...message, content: 'Outro texto' }, 'message-a')

  expect(reused).toMatchObject({ success: false, kind: 'request-id-reused' })
  expect(mockAxiosPost).toHaveBeenCalledTimes(1)
})

test('same request ID with a different normalized actor is rejected atomically', async () => {
  await sendWithRequestId(message, 'message-a')
  const reused = await sendWithRequestId({ ...message, sentBy: 'other-reviewer@example.test' }, 'message-a')

  expect(reused).toMatchObject({ success: false, kind: 'request-id-reused' })
  expect(mockAxiosPost).toHaveBeenCalledTimes(1)
})

test('provider success followed by audit failure becomes indeterminate and blocks retry', async () => {
  const createLog = jest.spyOn(DiscordMessageLog, 'create')
    .mockRejectedValueOnce(new Error('audit write failed'))

  const first = await sendWithRequestId(message, 'message-a')
  createLog.mockRestore()
  const retry = await sendWithRequestId(message, 'message-b')

  expect(first).toMatchObject({ success: false, kind: 'indeterminate' })
  expect(retry).toMatchObject({ success: false, kind: 'indeterminate' })
  expect(mockAxiosPost).toHaveBeenCalledTimes(1)
})

test('concurrent request IDs share one provider owner', async () => {
  let started = 0
  let release!: () => void
  ...
  mockAxiosPost.mockImplementationOnce(async () => {
    ...
    return { data: { messageIds: ['message-1'], parts: 1 } }
  })

  const firstPromise = sendWithRequestId(message, 'message-a')
  ...
  const second = await sendWithRequestId(message, 'message-b')
  release()
  const first = await firstPromise

  expect(second).toMatchObject({ success: false, kind: 'in-progress' })
  expect(first.success).toBe(true)
  expect(mockAxiosPost).toHaveBeenCalledTimes(1)
})

test('expired execution lease becomes indeterminate without reopening provider I/O', async () => {
  const now = new Date()
  await DiscordMessageExecutionReceipt.create({
    operation: 'manual-send',
    identity: discordMessageIdentity(message),
    requestId: 'stale-owner-request',
    ownerId: 'stale-owner',
    status: 'running',
  ...
  const result = await sendWithRequestId(message, 'new-request-after-stale', { now: () => now })

  expect(result).toMatchObject({ success: false, kind: 'indeterminate' })
  expect(mockAxiosPost).not.toHaveBeenCalled()
  expect(await DiscordMessageExecutionReceipt.findOne({ identity: discordMessageIdentity(message) }).lean())
    .toMatchObject({ status: 'indeterminate', providerStatus: 'unknown' })
})

test('scheduler without a request header fences the same rule and month', async () => {
  ...
  const firstPromise = runScheduled(undefined, { now: () => now })
  await new Promise<void>((resolve) => {
    const timer = setInterval(() => {
      if (started > 0) {
        clearInterval(timer)
  ...
  const secondPromise = runScheduled(undefined, { now: () => now })
  release()
