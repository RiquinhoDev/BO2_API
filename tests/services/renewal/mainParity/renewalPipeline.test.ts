...
import {
  ...
  type RenewalPipelineDependencies,
} from '../../../../src/services/renewal/renewalPipeline.service'

const executarPipeline = (dependencias: unknown) =>
  runRenewalPipelineComDependencias(dependencias as RenewalPipelineDependencies)
...
test('espelho parcial bloqueia todas as fases dependentes que escrevem', async () => {
  const fixtures = dependencias(true)
  fixtures.valor.syncActiveStudentSalesHistory = async () => {
    fixtures.ordem.push('hotmart')
    return { errors: [{ email: 'aluno@example.com', error: 'provider incompleto' }] }
  }

  const report = await executarPipeline(fixtures.valor)

  assert.deepEqual(fixtures.ordem, ['hotmart', 'ac', 'tags', 'discord'])
  assert.equal(report.hotmartSales.success, false)
  assert.equal(report.acExpiration.skipped, true)
  assert.equal(report.acTurmaTags.skipped, true)
  assert.equal(report.acRefunds.skipped, true)
  assert.equal(report.timelines.skipped, true)
  assert.equal(report.acPurchaseDate.skipped, true)
  assert.equal(report.success, false)
})

test('espelho com success false bloqueia fases dependentes', async () => {
  const fixtures = dependencias(true)
  fixtures.valor.syncActiveStudentAcRenewalData = async () => {
    fixtures.ordem.push('ac')
    return { success: false }
  }

  const report = await executarPipeline(fixtures.valor)

  assert.equal(report.acRenewalData.success, false)
  assert.equal(report.acExpiration.skipped, true)
  assert.equal(report.timelines.skipped, true)
  assert.equal(report.success, false)
})
