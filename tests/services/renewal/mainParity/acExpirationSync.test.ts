...
import * as acExpirationSync from '../../../../src/services/renewal/acExpirationSync.service'
import { TURMA_1_RENEWAL_OFFER_CODE, TURMA_2_RENEWAL_OFFER_CODE } from '../../../../src/services/renewal/renewalConstants'
...
const jestAfterCallbacks: Array<() => void | Promise<void>> = []
const t = { after: (callback: () => void | Promise<void>) => jestAfterCallbacks.push(callback) }
afterEach(async () => {
  while (jestAfterCallbacks.length > 0) await jestAfterCallbacks.pop()?.()
})
...
const queryComSelecao = <T extends Record<string, unknown>>(entries: T[]) => {
  ...
  const chain = {
    ...
    const seleccionados = new Set(campos.split(/\s+/).filter(Boolean))
      selected = entries.map((entrada) => Object.fromEntries(
    ...
    lean: () => chain,
    exec: async () => selected,
    cursor: () => (async function * () { yield * selected })(),
  }
  return chain
}
...
function instalarFixturesSync(
  ...
  const filtrosAc: any[] = []
  const hotmartBatchSizes: number[] = []
  let claims = 0
  ...
  ;(HotmartSaleHistory as any).find = (filtro: any = {}) => {
    if (filtro.userId?.$in) hotmartBatchSizes.push(filtro.userId.$in.length)
    return query(hotmartDocs.filter((entrada) => !filtro.userId?.$in || filtro.userId.$in.some((id: unknown) => String(id) === String(entrada.userId))))
  }
...
test('perda de lease antes do claim bloqueia estado, rasto e escrita AC da expiração', async () => {
  const compra = new Date('2026-05-20T09:00:00.000Z')
  const fixtures = instalarFixturesSync(
    [alunoAc({ purchaseDate: compra, expirationDate: null })],
    [alunoHotmart(compra)]
  )
  t.after(fixtures.restaurar)

  const report = await runWithMainParityPhaseHooks({
    assertOwnership: () => { throw new Error('lease lost before expiration claim') },
  ...
  assert.deepEqual(fixtures.estados, [])
  assert.deepEqual(fixtures.logs, [])
  assert.deepEqual(fixtures.escritas, [])
})

test('carrega fanouts em lotes de no máximo 200 sem truncar candidatos', async () => {
  const acEntries = Array.from({ length: 201 }, (_, i) => alunoAc({
    userId: `aluno-${i}`,
    email: `aluno-${i}@example.com`,
  }))
  const fixtures = instalarFixturesSync(acEntries, [], [])
  t.after(fixtures.restaurar)

  const report = await acExpirationSync.syncAcExpirationDates()

  assert.equal(report.skippedNoHotmartData, 201)
  assert.deepEqual(fixtures.hotmartBatchSizes, [200, 1])
})
...
test('syncAcExpirationDates conta encurtaria e diverge mesmo com o gatilho alinhado', async () => {
...
test('syncAcExpirationDates em dry-run por defeito só reporta a escrita que faria', async () => {
...
test('syncAcExpirationDates trata vendas apenas reembolsadas como reembolso Hotmart', async () => {
  const compra = new Date('2025-05-15T00:00:00Z')
  const fixtures = instalarFixturesSync(
    [alunoAc()],
    [
...
test('reembolso sinalizado pela AC deixa uma recusa exacta e idempotente sem chamar a AC', async () => {
  const fixtures = instalarFixturesSync(
    [alunoAc({
...
test('estado Reembolsada sem refundDate não cria nova recusa em fotografias AC posteriores', async () => {
...
test('refund e chargeback sinalizados pela Hotmart deixam uma recusa por evento em dry-run', async () => {
...
test('syncAcExpirationDates reporta encurtaria mas não avança sem latestApprovedDate', async () => {
  const compra = new Date('2025-05-15T00:00:00Z')
  const fixtures = instalarFixturesSync(
...
test('syncAcExpirationDates só conta falta de latestApprovedDate depois de avaliar divergência segura', async () => {
  const compra = new Date('2025-05-15T00:00:00Z')
  const fixtures = instalarFixturesSync(
...
test('syncAcExpirationDates só escreve quando dryRun é explicitamente falso', async () => {
  const compraAntiga = new Date('2025-05-15T00:00:00Z')
  const compraNova = new Date('2026-05-15T00:00:00Z')
  const fixtures = instalarFixturesSync(
    [alunoAc({ purchaseDate: compraAntiga })],
...
test('bootstrap real com expiração preenchida grava só o watermark sem chamar a AC', async () => {
...
test('segunda corrida real sem evento novo escreve zero', async () => {
  const compra = new Date('2026-05-15T00:00:00Z')
  const fixtures = instalarFixturesSync(
...
test('evento já certo na AC avança o watermark sem escrever', async () => {
  const compra = new Date('2026-05-15T00:00:00Z')
  const fixtures = instalarFixturesSync(
    [alunoAc({ expirationDate: new Date('2027-05-31T23:59:59.999Z') })],
    [alunoHotmart(compra)]
...
test('falta de contacto mantém o evento pendente', async () => {
  const compra = new Date('2026-05-15T00:00:00Z')
  const fixtures = instalarFixturesSync(
...
test('uma renovação nova dispara apenas o aluno cujo ciclo mudou', async () => {
...
test('uma prestação posterior do mesmo ciclo não cria evento', async () => {
...
test('uma extensão que muda o ciclo de um para dois anos cria evento', async () => {
...
test('expiração vazia continua elegível durante o bootstrap', async () => {
...
test('o mesmo ciclo tratado abre um episódio para uma fotografia AC vazia', async () => {
...
test('concorrência e repetição da mesma fotografia AC vazia não duplicam a escrita', async () => {
...
test('uma fotografia AC vazia posterior abre um novo episódio observável', async () => {
...
test('syncError não abre episódio vazio nem consome a recuperação da leitura seguinte', async () => {
...
test('confirmação externa pendente não reabre com uma fotografia AC vazia posterior', async () => {
...
test('execução manual por email não varre os outros alunos', async () => {
...
test('dry-run não chama a AC nem avança o watermark de um evento novo', async () => {
...
test('falha da AC não avança o watermark e a corrida seguinte tenta de novo', async () => {
...
test('duas corridas simultâneas reclamam o evento uma vez e fazem uma chamada à AC', async () => {
...
test('o contender tardio perde o claim quando a primeira corrida já finalizou o mesmo evento', async () => {
...
test('preencher a transação da mesma âncora não cria um evento falso', async () => {
...
test('um ciclo genuinamente novo usa a transação como chave canónica da venda', async () => {
...
test('sucesso na AC com falha de finalização fica pendente e reconcilia sem duplicar', async () => {
  const compra = new Date('2026-12-05T00:00:00Z')
  const acEntries: any[] = [alunoAc({ expirationDate: new Date('2026-12-31T23:59:59.999Z') })]
  const fixtures = instalarFixturesSync(
    acEntries,
    [alunoHotmart(compra)],
    [oferta()],
...
test('falha de CAS num aluno é reportada e não impede o bootstrap do seguinte', async () => {
...
test('falha ao finalizar bootstrap fica interna e refinaliza sem esperar a fotografia AC', async () => {
...
test('uma fotografia antiga nunca faz regredir a âncora já tratada', async () => {
...
test('chamada manual pode retomar um claim expirado que a libertação não removeu', async () => {
...
test('retoma manual nunca faz o pending regredir de dois para um ano', async () => {
...
test('dry-run observa uma confirmação pendente sem finalizar o watermark', async () => {
...
test('compra base preserva os dois anos do nome e escreve a expiração do período da oferta', async () => {
...
test('aluno em turma base datável usa a turma quando a oferta não tem nome', async () => {
...
test('aluno em turma de renovação datável usa a compra quando a oferta não tem nome', async () => {
...
test('aluno sem turma usa a oferta nomeada como recurso', async () => {
  const compra = new Date('2025-07-10T00:00:00Z')
  const fixtures = instalarFixturesSync(
    [alunoAc()],
...
test('código da linhagem Turma 1 usa renovação mesmo numa turma base datável', async () => {
...
test('códigos reais das Turmas 1 e 2 usam compra mais anos mesmo com nome base e isRenewal falso', async () => {
...
test('nome classificado como renovação escreve os dois anos do ciclo', async () => {
...
test('ciclo de dois anos com a expiração vazia não cai no cálculo de um ano', async () => {
...
test('oferta base sem nome e período válidos não escreve e conta semTurma', async () => {
  const compra = new Date('2026-03-04T00:00:00Z')
  const fixtures = instalarFixturesSync(
    [alunoAc()],
...
test('encurtaria recusa escritas calculadas pelos ramos base e renovação', async () => {
...
test('a compra âncora, não a última prestação, escolhe a oferta e a data', async () => {
...
test('332 regista recusas encurtaria, semVenda e semTurma com valores exactos', async () => {
...
test('332 cria o rasto antes da AC e conserva escrito em sucesso', async () => {
...
test('falha ao criar rasto do 332 bloqueia a AC e liberta o claim', async () => {
...
test('false e throw da AC transformam a intenção do 332 em recusado', async () => {
  const compra = new Date('2026-08-20T00:00:00.000Z')
  const fixtures = instalarFixturesSync(
    [
...
test('duas corridas concorrentes de encurtaria criam uma única recusa depois do claim', async () => {
...
test('dry-run repetido da expiração grava uma única proposta determinística', async () => {
...
test('semVenda e semTurma repetidos da expiração gravam uma recusa por snapshot/evento', async () => {
