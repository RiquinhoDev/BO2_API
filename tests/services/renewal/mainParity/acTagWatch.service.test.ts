...
const SERVICO = path.resolve(__dirname, '../../../../src/services/renewal/acTagWatch.service.ts')
const fonte = fs.readFileSync(SERVICO, 'utf8')
const contexto = fs.readFileSync(path.join(path.dirname(SERVICO), 'acTagWatch.context.ts'), 'utf8')
...
test('o diff le o espelho antes de qualquer escrita nele', () => {
  const posLeitura = fonte.indexOf('ACStudentTag.find({})')
...
test('"activo" e combined.status — o userproducts diz QUEM e de OGI, nao se esta activo', () => {
  ...
  assert.match(contexto, /'combined\.status': 'ACTIVE'/)
  assert.match(contexto, /status: 'ACTIVE'/, 'o userproducts filtra a coorte OGI')
  assert.match(contexto, /Grande Investimento/, 'tem de identificar o produto OGI')
})

test('sem produto OGI a corrida para, em vez de medir contra uma coorte vazia', () => {
  assert.match(contexto, /Produto OGI activo não encontrado/)
})

test('um dry-run nosso nao protege — so escritas reais contam como nossas', () => {
  assert.match(fonte, /dryRun: false,\s*servico: \{ \$in: \['turmaTag', 'reembolso'\] \}/)
})

test('um periodo so conta como pago se a compra nao foi reembolsada', () => {
  assert.match(contexto, /x\?\.reembolsada !== true/)
})
