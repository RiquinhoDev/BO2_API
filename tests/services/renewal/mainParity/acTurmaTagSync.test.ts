...
import { decidirTurmaTag } from '../../../../src/services/renewal/acTurmaTagSync.service'
...
test('dry-run por omissão não chama a AC', async () => {
  const originals = {
    ...
    addTag: (activeCampaignService as any).addTag
  }
  ...
  ;(activeCampaignService as any).addTag = async () => { chamadasAc += 1; return { success: true } }
  try {
    ...
    ;(activeCampaignService as any).addTag = originals.addTag
  }
})
...
test('resolvedor convencional usa a tag confirmada na AC', async () => {
  ...
  try {
    ...
    assert.equal(logs.length, 0, 'dry-run não persiste rasto')
  } finally {
    ;(StudentRenewalTimeline as any).find = originals.timeline
    ;(TurmaTagMap as any).find = originals.mapa
    ;(ACStudentTag as any).find = originals.tags
...
test('escrita de tag marca tentativa provider antes do POST e sucesso depois', async () => {
  const originals = {
    timeline: (StudentRenewalTimeline as any).find,
    mapa: (TurmaTagMap as any).find,
    tags: (ACStudentTag as any).find,
  ...
  const ordem: string[] = []
  ;(StudentRenewalTimeline as any).find = () => mockQuery([{
    email: 'hooks@example.com',
    ciclos: [{ turma: { nome: 'Turma 18 | 2605' }, compras: [{ reembolsada: false }] }]
  }])
  ;(TurmaTagMap as any).find = () => mockQuery([])
  ;(ACStudentTag as any).find = () => mockQuery([{ email: 'hooks@example.com', contactId: 'c18', tags: [] }])
  ;(User as any).find = () => mockQuery([{ email: 'hooks@example.com', combined: { status: 'ACTIVE' } }])
  ;(AcWriteLog as any).create = async () => undefined
  ;(activeCampaignService as any).findExistingTagByName = async () => '1805'
  Object.defineProperty(activeCampaignService, 'client', {
    configurable: true,
    value: { post: async () => { ordem.push('post'); return {} } },
  })
  try {
    const report = await runWithMainParityPhaseHooks({
      assertOwnership: () => undefined,
      localMutationStarted: () => { ordem.push('local') },
      providerStarted: () => { ordem.push('provider') },
      providerSucceeded: () => { ordem.push('success') },
    }, () => syncTurmaTags({ dryRun: false }))

    assert.equal(report.aplicadas, 1)
    assert.deepEqual(ordem, ['local', 'provider', 'post', 'success'])
  } finally {
    ;(StudentRenewalTimeline as any).find = originals.timeline
    ;(TurmaTagMap as any).find = originals.mapa
    ;(ACStudentTag as any).find = originals.tags
    ...
    ;(AcWriteLog as any).create = originals.log
    ;(activeCampaignService as any).findExistingTagByName = originals.findTag
    delete (activeCampaignService as unknown as { client?: unknown }).client
  }
})
