...
import { getProductStats as extractedHandler } from '../../src/services/users/userProductStats.runtime'

type Handler = (req: Request, res: Response, next: NextFunction) => Promise<void>
const getProductStats = extractedHandler as unknown as Handler
...
describe('GET /api/users/getProductStats — product membership stats characterization', () => {
  ...
  it('reports failure through next(HttpError) with PRODUCT_MEMBERSHIP_STATS_FAILED', async () => {
    jest.spyOn(User, 'find').mockImplementation((() => { throw new Error('boom') }) as never)

    const captured: Captured = {}
    const next = jest.fn()
    await getProductStats(req, makeResponse(captured), next as unknown as NextFunction)

    expect(captured.body).toBeUndefined()
    expect(next).toHaveBeenCalledTimes(1)
    const error = next.mock.calls[0]?.[0] as HttpError
    expect(error).toBeInstanceOf(HttpError)
    expect(error).toMatchObject({ status: 500, code: 'PRODUCT_MEMBERSHIP_STATS_FAILED' })
