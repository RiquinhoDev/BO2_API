...
const selected: Array<[string, number]> = [
  ...
  ['src/controllers/guruAnalytics/churn.controller.ts', 4],
  ...
  ['src/controllers/guru.trials.controller.ts', 2],
]
...
test('excluded Guru identities keep their existing behavior', () => {
  expect(read('src/controllers/guruAnalytics/churn.controller.ts'))
    .toMatch(/return res\.json\(\{\s*success: true,\s*mrr:/)
  expect(read('src/controllers/guru.trials.controller.ts').match(/successResponse\(/g))
    .toHaveLength(2)
