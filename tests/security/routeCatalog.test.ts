...
test('o catalogo cobre exatamente as 438 rotas do manifest', () => {
  expect(manifest).toHaveLength(438)
  expect(catalog).toHaveLength(438)
  expect(new Set(manifest.map(key)).size).toBe(438)
  expect(new Set(catalog.map(key)).size).toBe(438)
...
test('a superficie excecional fica curta e explicita', () => {
  ...
  expect(routesWith('authenticated')).toHaveLength(428)
