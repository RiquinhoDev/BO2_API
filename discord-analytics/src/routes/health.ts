...
router.get('/', async (req, res) => {
  try {
    ...
    // Log do health check
    logger.info(`🏥 Health check: ${overallStatus} (${responseTime}ms)`);
...
async function checkDatabaseHealth(): Promise<DatabaseHealth> {
  try {
    ...
    logger.error('❌ Erro ao verificar saúde do database:', error);
    return {
      status: 'error',
      collections: { activities: 0, voice: 0, engagement: 0 }
    };
...
async function checkBotHealth(bot: any): Promise<BotHealth> {
  try {
    if (!bot || !bot.client) {
      return {
        status: 'offline',
    ...
    logger.error('❌ Erro ao verificar saúde do bot:', error);
    return {
      status: 'offline',
      guilds: 0,
      users: 0,
