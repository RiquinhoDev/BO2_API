...
router.get('/overview', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`📊 Buscando overview para guild ${guildId}, últimos ${days} dias`);
        const overview = await AnalyticsCollector_1.AnalyticsCollector.getServerOverview(guildId, parseInt(days));
        if (overview) {
        ...
        logger_1.logger.error('❌ Erro em /analytics/overview:', error);
        res.status(500).json({
            success: false,
...
router.get('/messages', async (req, res) => {
    try {
        const { days = 7, guildId = process.env.DISCORD_GUILD_ID, userId, channelId } = req.query;
        ...
        if (channelId)
            matchFilter.channels = channelId;
        // Agregação por dia
        const dailyStats = await DiscordActivity_1.DiscordActivity.aggregate([
            { $match: matchFilter },
            {
                $group: {
                    _id: '$date',
        ...
        logger_1.logger.error('❌ Erro em /analytics/messages:', error);
        res.status(500).json({
            success: false,
            error: 'Erro interno do servidor'
        });
...
router.get('/voice', async (req, res) => {
    try {
        ...
        const topChannels = await VoiceActivity_1.VoiceActivity.aggregate([
            {
                $match: {
                    guildId,
                    joinTime: { $gte: startDate },
        ...
        logger_1.logger.error('❌ Erro em /analytics/voice:', error);
        res.status(500).json({
            success: false,
            error: 'Erro interno do servidor'
        });
...
router.get('/engagement', async (req, res) => {
    try {
        ...
        const engagementDistribution = await UserEngagement_1.UserEngagement.aggregate([
            {
                $match: {
                    guildId,
        ...
        const topUsers = await UserEngagement_1.UserEngagement.aggregate([
            {
                $match: {
                    guildId,
                    date: { $gte: new Date(dateStr) }
        ...
        logger_1.logger.error('❌ Erro em /analytics/engagement:', error);
        res.status(500).json({
            success: false,
            error: 'Erro interno do servidor'
        });
...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        const userActivity = await DiscordActivity_1.DiscordActivity.find({
            userId,
            guildId,
            date: { $gte: dateStr }
        }).sort({ date: 1 });
        // Engagement do utilizador
        const userEngagement = await UserEngagement_1.UserEngagement.find({
            userId,
            guildId,
            date: { $gte: new Date(dateStr) }
        }).sort({ date: 1 });
        // Atividade de voz
        const voiceActivity = await VoiceActivity_1.VoiceActivity.find({
            userId,
            guildId,
            joinTime: { $gte: startDate }
        }).sort({ joinTime: 1 });
        ...
        logger_1.logger.error(`❌ Erro em /analytics/user/${req.params.userId}:`, error);
        res.status(500).json({
            success: false,
            error: 'Erro interno do servidor'
        });
...
router.get('/users', async (req, res) => {
    try {
        const { days = 7, guildId = process.env.DISCORD_GUILD_ID, limit = 100, sortBy = 'messages' // messages, voice, engagement
         } = req.query;
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - parseInt(days));
        const dateStr = startDate.toISOString().split('T')[0];
        logger_1.logger.info(`🔍 Buscando utilizadores para guild ${guildId} desde ${dateStr}`);
        ...
        const users = await DiscordActivity_1.DiscordActivity.aggregate([
            {
                $match: {
                    guildId,
                    date: { $gte: dateStr }
        ...
        logger_1.logger.info(`📊 Encontrados ${users.length} utilizadores com atividade`);
        // Buscar dados de engagement para cada utilizador
        const engagementData = await UserEngagement_1.UserEngagement.find({
            guildId,
        ...
        logger_1.logger.error('❌ Erro em /analytics/users:', error);
        res.status(500).json({
            success: false,
            error: 'Erro interno do servidor'
        });
...
router.get('/debug', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`🔍 Debug de dados para guild ${guildId}`);
        // Contar documentos na coleção DiscordActivity
        const totalActivities = await DiscordActivity_1.DiscordActivity.countDocuments({ guildId });
        const messageActivities = await DiscordActivity_1.DiscordActivity.countDocuments({
            guildId,
            type: 'message'
        });
        const voiceActivities = await DiscordActivity_1.DiscordActivity.countDocuments({
            guildId,
            type: 'voice'
        });
        // Buscar algumas atividades recentes
        const recentActivities = await DiscordActivity_1.DiscordActivity.find({ guildId })
            .sort({ lastActivity: -1 })
            .limit(5)
            .select('userId username displayName type count totalWords lastActivity');
        // Buscar utilizadores únicos
        const uniqueUsers = await DiscordActivity_1.DiscordActivity.distinct('userId', { guildId });
        res.json({
            success: true,
            data: {
                guildId,
        ...
        logger_1.logger.error('❌ Erro em /analytics/debug:', error);
        res.status(500).json({
            success: false,
            error: 'Erro interno do servidor',
            message: error.message
...
router.post('/refresh', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`🔄 Iniciando refresh manual das analytics para guild ${guildId}`);
        ...
        logger_1.logger.error('❌ Erro em /analytics/refresh:', error);
        res.status(500).json({
            success: false,
            error: 'Erro interno do servidor'
        });
