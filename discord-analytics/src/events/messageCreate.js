    ...
    try {
        ...
        const getChannelName = (channel) => {
            try {
                ...
                logger_1.logger.warn(`⚠️ Erro ao obter nome do canal ${channel.id}:`, error);
        ...
        try {
            await AnalyticsCollector_1.AnalyticsCollector.saveMessageActivity(messageData);
            logger_1.logger.info(`✅ Mensagem salva: ${messageData.username} - ${messageData.wordCount} palavras`);
        }
        catch (error) {
        ...
        if (process.env.NODE_ENV === 'development') {
            logger_1.logger.debug(`📝 Mensagem de ${messageData.username} em #${messageData.channelName} - ${messageData.wordCount} palavras`, {
        ...
        if (process.env.NODE_ENV === 'production') {
            logger_1.logger.info(`📝 Mensagem processada: ${messageData.username} em #${messageData.channelName}`);
        }
    }
    catch (error) {
        // Log detalhado do erro para debugging
        logger_1.logger.error('❌ Erro ao processar messageCreate:', {
            error: error.message,
            stack: error.stack,
            messageId: message?.id,
            channelId: message?.channel?.id,
