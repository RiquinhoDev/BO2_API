...
// Interface para request com autenticação
...
export function validateApiKey(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (!expectedKey) {
      logger.warn('⚠️ API_SECRET_KEY não configurada - acesso sem autenticação');
    ...
    if (!apiKey) {
      logger.warn(`🔒 Tentativa de acesso sem API key de ${req.ip}`);
    ...
    if (apiKey !== expectedKey) {
      logger.warn(`🔒 Tentativa de acesso com API key inválida de ${req.ip}: ${apiKey.substring(0, 8)}...`);
      return res.status(401).json({
        success: false,
    ...
    logger.debug(`✅ Acesso autorizado de ${req.ip}`);
...
export function optionalAuth(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (apiKey && expectedKey && apiKey === expectedKey) {
      req.isAuthenticated = true;
      req.apiKey = apiKey;
      logger.debug(`✅ Acesso autenticado de ${req.ip}`);
    } else {
      req.isAuthenticated = false;
      logger.debug(`ℹ️ Acesso não autenticado de ${req.ip}`);
    }
...
export function requireAdmin(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (adminKeys.length > 0 && !adminKeys.includes(req.apiKey || '')) {
      logger.warn(`🔒 Tentativa de acesso admin sem permissão de ${req.ip}`);
    ...
    logger.debug(`👑 Acesso admin autorizado de ${req.ip}`);
    next();
...
export function logAccess(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  res.send = function(body) {
    ...
    logger.info(`${authenticated} ${method} ${url} - ${status} - ${duration}ms - ${ip} - ${userAgent}`);
