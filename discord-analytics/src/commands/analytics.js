...
const EngagementCalculator_1 = require("../services/EngagementCalculator");
...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .setRequired(true)
    .addChoices({ name: 'Mensagens', value: 'messages' }, { name: 'Voz', value: 'voice' }, { name: 'Engagement', value: 'engagement' }))
...
async function execute(interaction) {
    try {
        ...
        logger_1.logger.error('❌ Erro ao executar comando analytics:', error);
        const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
        try {
            await interaction.editReply(`❌ Erro ao executar comando: ${errorMessage}`);
        }
        catch (replyError) {
            logger_1.logger.error('❌ Erro ao enviar resposta de erro:', replyError);
        }
    }
}
...
async function handleOverview(interaction) {
    ...
    const overview = await AnalyticsCollector_1.AnalyticsCollector.getServerOverview(guildId, days);
    if (!overview) {
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        name: '💬 Mensagens',
        value: [
            `**Total:** ${helpers_1.helpers.format.formatNumber(overview.messages.total)}`,
        ...
        name: '🎤 Voz',
        value: [
            `**Tempo Total:** ${helpers_1.helpers.format.formatDuration(overview.voice.totalMinutes)}`,
...
async function handleUser(interaction) {
    ...
    const userEngagement = await EngagementCalculator_1.EngagementCalculator.calculateUserEngagement(targetUser.id, guildId, days);
    const userTrend = await EngagementCalculator_1.EngagementCalculator.calculateTrend(targetUser.id, guildId);
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        value: [
            ...
            `**Nível:** ${EngagementCalculator_1.EngagementCalculator.getEngagementLevel(userEngagement.total)}`,
...
async function handleTop(interaction) {
    ...
    switch (category) {
        case 'engagement':
            ...
            topUsers = await EngagementCalculator_1.EngagementCalculator.getEngagementRankings(guildId, limit);
            break;
...
async function handleRefresh(interaction) {
    ...
    try {
        await EngagementCalculator_1.EngagementCalculator.calculateAllUsersEngagement(guildId);
        await interaction.followUp({
            content: '✅ Refresh de engagement concluído com sucesso!',
            ephemeral: true
        });
        logger_1.logger.info(`🔄 Refresh de engagement executado por ${interaction.user.username} (${interaction.user.id})`);
    }
    catch (error) {
        logger_1.logger.error('❌ Erro durante refresh de engagement:', error);
        await interaction.followUp({
