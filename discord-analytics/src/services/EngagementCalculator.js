...
exports.EngagementCalculator = void 0;
...
class EngagementCalculator {
    ...
    static async calculateUserEngagement(userId, guildId, days = 7) {
        try {
            const startDate = new Date();
            startDate.setDate(startDate.getDate() - days);
            const dateStr = startDate.toISOString().split('T')[0];
            // Buscar atividades do utilizador
            const activities = await DiscordActivity_1.DiscordActivity.find({
                userId,
                guildId,
                date: { $gte: dateStr }
            });
            // Buscar atividade de voz
            const voiceActivities = await VoiceActivity_1.VoiceActivity.find({
                userId,
                guildId,
                date: { $gte: dateStr },
            ...
            logger_1.logger.error('❌ Erro ao calcular engagement do utilizador:', error);
            return { total: 0, messages: 0, voice: 0, presence: 0 };
        }
    }
    // 💬 CALCULAR SCORE DE MENSAGENS
    static calculateMessageScore(activities) {
    ...
    static calculateVoiceScore(voiceActivities) {
        ...
        if (totalSessions > 0) {
            ...
            if (avgSessionDuration >= 30)
                score += totalSessions * 1; // Bonus para sessões longas
            if (avgSessionDuration >= 60)
                score += totalSessions * 0.5; // Bonus extra
        }
    ...
    static calculatePresenceScore(activities) {
        const presenceActivities = activities.filter(a => a.type === 'presence');
        if (presenceActivities.length === 0)
            return 0;
        let score = 0;
    ...
    static async calculateTrend(userId, guildId) {
        try {
            ...
            const previousActivities = await DiscordActivity_1.DiscordActivity.find({
                userId,
                guildId,
                date: {
            ...
            const previousVoiceActivities = await VoiceActivity_1.VoiceActivity.find({
                userId,
                guildId,
                date: {
                    $gte: previousStart.toISOString().split('T')[0],
            ...
            logger_1.logger.error('❌ Erro ao calcular tendência:', error);
    ...
    static async calculateAllUsersEngagement(guildId) {
        try {
            logger_1.logger.info('🔄 Iniciando cálculo de engagement para todos os utilizadores...');
            ...
            logger_1.logger.info(`📊 Processando engagement para ${activeUsers.length} utilizadores...`);
            ...
            for (let i = 0; i < activeUsers.length; i += batchSize) {
                ...
                await Promise.all(batch.map(async (userId) => {
                    try {
                        ...
                        const recentActivity = await DiscordActivity_1.DiscordActivity.findOne({
                            userId,
                            guildId,
                        ...
                        logger_1.logger.warn(`⚠️ Erro ao calcular engagement para ${userId}:`, error);
                    }
                }));
                logger_1.logger.info(`✅ Processado batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(activeUsers.length / batchSize)}`);
            }
            logger_1.logger.info('✅ Cálculo de engagement concluído para todos os utilizadores');
        }
        catch (error) {
            logger_1.logger.error('❌ Erro ao calcular engagement para todos os utilizadores:', error);
        }
    }
    // 📈 OBTER RANKINGS DE ENGAGEMENT
    static async getEngagementRankings(guildId, limit = 20) {
        try {
            return await UserEngagement_1.UserEngagement.find({ guildId })
                .sort({ currentScore: -1 })
                .limit(limit)
                .select({
                userId: 1,
            ...
            logger_1.logger.error('❌ Erro ao obter rankings de engagement:', error);
            return [];
        }
    }
    // 📊 OBTER ESTATÍSTICAS DE ENGAGEMENT
    static async getEngagementStats(guildId) {
        try {
            const stats = await UserEngagement_1.UserEngagement.aggregate([
                { $match: { guildId } },
                {
                    $group: {
                        _id: null,
            ...
            logger_1.logger.error('❌ Erro ao obter estatísticas de engagement:', error);
            return null;
        }
    }
}
exports.EngagementCalculator = EngagementCalculator;
