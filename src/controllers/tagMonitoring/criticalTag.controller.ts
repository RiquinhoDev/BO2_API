...
export const addCriticalTag = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(201).json(successResponse(tag, { message: 'Tag crítica adicionada com sucesso' }))
  } catch (error: unknown) {
...
export const removeCriticalTag = async (req: Request<CriticalTagParams>, res: Response, next: NextFunction) => {
  try {
    ...
    res.json(successResponse(null, { message: 'Tag crítica removida com sucesso' }))
  } catch (error: unknown) {
    if (error instanceof Error) {
      const { message } = error
...
export const toggleCriticalTag = async (req: Request<CriticalTagParams>, res: Response, next: NextFunction) => {
  try {
    ...
    res.json(successResponse(tag, { message: `Tag crítica ${tag.isActive ? 'ativada' : 'desativada'} com sucesso` }))
  } catch (error: unknown) {
    if (error instanceof Error) {
      const { message } = error
      if (message.includes('não encontrada')) {
...
export const updateCriticalTagPriority = async (req: Request<CriticalTagParams>, res: Response, next: NextFunction) => {
  try {
    ...
    res.json(successResponse(tag, { message: `Prioridade atualizada para ${priority}` }))
  } catch (error: unknown) {
    if (error instanceof Error) {
      const { message } = error
      if (message.includes('não encontrada')) {
...
export const getAvailableNativeTags = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.json(successResponse(tags, { count: tags.length, weeksAnalyzed: weeks }))
  } catch (error: unknown) {
