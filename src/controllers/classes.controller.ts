...
class ClassesController {
  ...
  // Cria/atualiza várias turmas de uma vez — para pré-registar turmas que
  // já foram criadas na Hotmart mas ainda não têm nenhum aluno lá dentro
  // (o sync nortuno só descobre turmas através de alunos matriculados).
  bulkAddOrEditClasses = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!Array.isArray(classes) || classes.length === 0) {
        res.status(400).json({
          success: false,
          message: '"classes" tem de ser uma lista não vazia'
        })
        return
      }

      const results: Array<{ classId: string; success: boolean; isNew?: boolean; error?: string }> = []
      ...
      for (const entry of classes) {
        ...
        if (!classId || !name) {
          results.push({ classId: classId || '(vazio)', success: false, error: 'classId e name são obrigatórios' })
          continue
        }

        try {
          const isActive = entry.isActive !== undefined ? entry.isActive : true
          const finalEstado = entry.estado || (isActive ? 'ativo' : 'inativo')
          ...
          results.push({ classId, success: true, isNew: result.isNew })
        } catch (error) {
          results.push({ classId, success: false, error: (error as Error).message })
        }
      }
      ...
      res.json({
        success: failed === 0,
        message: `${created} criadas, ${updated} atualizadas, ${failed} falharam`,
        results,
        summary: { created, updated, failed },
        timestamp: new Date().toISOString()
      })
    } catch (error) {
      console.error('❌ Erro ao criar turmas em lote:', error)
      res.status(500).json({
        success: false,
        message: 'Erro ao processar turmas em lote',
        error: (error as Error).message
      })
    }
  }
