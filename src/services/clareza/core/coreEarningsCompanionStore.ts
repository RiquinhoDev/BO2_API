import ClarezaCoreEarningsCompanion, { type IClarezaCoreEarningsCompanion } from '../../../models/ClarezaCoreEarningsCompanion'
...
export class MongooseCoreEarningsCompanionStore implements CoreEarningsCompanionStore {
  async read(generationId: string): Promise<CoreEarningsCompanionGeneration | null> {
    const found: IClarezaCoreEarningsCompanion[] = []
    const cursor = ClarezaCoreEarningsCompanion.find({ generationId })
      .sort({ _id: 1 })
      .maxTimeMS(5_000)
      .lean()
  ...
  async replace(collection: CoreEarningsCompanionCollection): Promise<void> {
    ...
    if (operations.length) {
      ...
      await ClarezaCoreEarningsCompanion.bulkWrite(operations, { ordered: false })
    }
    beforeCanonicalMutation()
