import ClarezaCoreTop10Companion, { type IClarezaCoreTop10Companion } from '../../../models/ClarezaCoreTop10Companion'
...
export class MongooseCoreTop10CompanionStore implements CoreTop10CompanionStore {
  async read(generationId: string): Promise<CoreTop10CompanionGeneration | null> {
    const found: IClarezaCoreTop10Companion[] = []
    const cursor = ClarezaCoreTop10Companion.find({ generationId })
      .sort({ _id: 1 })
      .maxTimeMS(5_000)
      .lean()
  ...
  async replace(collection: CoreTop10CompanionCollection): Promise<void> {
    ...
    if (operations.length) {
      ...
      await ClarezaCoreTop10Companion.bulkWrite(operations, { ordered: false })
    }
    beforeCanonicalMutation()
