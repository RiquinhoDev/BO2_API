...
const MAX_WATCH_READ_ROWS = 20_000
...
export async function correrAcTagWatch(opcoes: AcTagWatchOpcoes = {}): Promise<AcTagWatchReport> {
  ...
  if (report.errors.length > 0) throw new Error('AC_TAG_WATCH_PROVIDER_READ_INCOMPLETE')
  report.alunosLidos = leitura.porEmail.size

  // 2. O espelho da véspera, ANTES de ser sobrescrito.
  const espelho = await collectCappedCursor(ACStudentTag.find({})
    .select('email contactId tags naListaAlunosOgi syncedAt')
    .sort({ _id: 1 })
    .maxTimeMS(5_000)
    .lean()
    .cursor({ batchSize: READ_BATCH_SIZE }), MAX_WATCH_READ_ROWS, 'AC_TAG_WATCH_MIRROR') as Array<{
      email: string
      contactId?: string
  ...
  for (const email of emails) {
    ...
    const antes: FotoTag[] = (antesDoc?.tags ?? []).map((t) => ({
      ...
      nome: String(t.nome),
      tipo: t.tipo === 'canonica' || t.tipo === 'membresia' ? t.tipo : 'outra',
  ...
  const nossas = await collectCappedCursor(AcWriteLog.find({
    dryRun: false,
    servico: { $in: ['turmaTag', 'reembolso'] },
    quando: { $gte: new Date(detectadoEm.getTime() - janelaNossa) },
  })
    .select('email tagId quando')
    .sort({ _id: 1 })
    .maxTimeMS(5_000)
    .lean()
    .cursor({ batchSize: READ_BATCH_SIZE }), MAX_WATCH_READ_ROWS, 'AC_TAG_WATCH_WRITE_LOG') as Array<{ email: string; tagId?: string | null; quando: Date }>
  ...
  const docs: Array<{ chave: string; [key: string]: unknown }> = []
  for (const e of comLote) {
  ...
  if (!dryRun && docs.length) {
    if (docs.length > 20_000) throw new Error('AC_TAG_EVENT_WRITE_CAP_EXCEEDED')
    mainParityLocalMutationStarted()
    for (let offset = 0; offset < docs.length; offset += 200) {
      assertMainParityOwnership()
      const ops: AnyBulkWriteOperation[] = docs.slice(offset, offset + 200).map((d) => ({
        updateOne: { filter: { chave: d.chave }, update: { $setOnInsert: d }, upsert: true }
      }))
      const r = await AcTagEvent.bulkWrite(ops, { ordered: true })
