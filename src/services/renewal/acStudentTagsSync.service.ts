...
import { readProviderArray } from './acProviderPage'

const DB_READ_BATCH_SIZE = 200
const MAX_MIRROR_ROWS = 20_000
...
type ReadBudget = { items: number }

function chargeReadBudget(budget: ReadBudget, count: number): void {
  budget.items += count
  if (budget.items > MAX_PROVIDER_READ_ITEMS) throw new Error('AC_TAG_READ_CAP_EXCEEDED')
}

async function todasAsTags(budget: ReadBudget): Promise<Array<{ id: string; tag: string }>> {
  const out: Array<{ id: string; tag: string }> = []
  let offset = 0
  while (true) {
    assertMainParityOwnership()
    const r = await axios.get<{ tags?: Array<{ id: string; tag: string }> }>(`${AC_URL()}/api/3/tags`, {
      headers: AC_HEADERS(),
      params: { limit: 100, offset },
      timeout: 45000
    })
    const t = readProviderArray<{ id: string; tag: string }>(r.data, 'tags', 100, 'AC_TAGS')
    chargeReadBudget(budget, t.length)
    out.push(...t)
    if (t.length < 100) break
    offset += 100
    if (offset >= 10_000) throw new Error('AC_TAG_CATALOG_READ_CAP_EXCEEDED')
  }
  return out
}

async function contactosDaTag(tagId: string, budget: ReadBudget): Promise<Array<{ id: string; email: string }>> {
  const out: Array<{ id: string; email: string }> = []
  let offset = 0
  while (true) {
    assertMainParityOwnership()
    const r = await axios.get<{ contacts?: Array<{ id: string; email?: string }> }>(`${AC_URL()}/api/3/contacts`, {
      headers: AC_HEADERS(),
      params: { tagid: tagId, limit: 100, offset },
      timeout: 45000
    })
    const cs = readProviderArray<{ id: string; email?: string }>(r.data, 'contacts', 100, 'AC_TAG_CONTACTS')
    chargeReadBudget(budget, cs.length)
    for (const c of cs) {
    ...
    offset += 100
    if (offset >= MAX_PROVIDER_READ_ITEMS) throw new Error('AC_TAG_CONTACT_READ_CAP_EXCEEDED')
 ...
 */
async function emailsDaLista(listId: string, budget: ReadBudget): Promise<Set<string>> {
  const out = new Set<string>()
  let offset = 0
  while (true) {
    assertMainParityOwnership()
    const r = await axios.get<{ contacts?: Array<{ email?: string }> }>(`${AC_URL()}/api/3/contacts`, {
      headers: AC_HEADERS(),
      params: { listid: listId, limit: 100, offset },
      timeout: 45000
    })
    const cs = readProviderArray<{ email?: string }>(r.data, 'contacts', 100, 'AC_LIST_CONTACTS')
    chargeReadBudget(budget, cs.length)
    for (const c of cs) {
      const email = String(c.email ?? '').toLowerCase().trim()
    ...
    if (cs.length < 100) break
    offset += 100
    if (offset >= MAX_PROVIDER_READ_ITEMS) throw new Error('AC_TAG_LIST_READ_CAP_EXCEEDED')
    await new Promise((r) => setTimeout(r, 100))
  }
  return out
}
 ...
 */
async function datasDasTagsDoContacto(contactId: string, budget: ReadBudget): Promise<Map<string, Date>> {
  assertMainParityOwnership()
  const r = await axios.get<{ contactTags?: Array<{ tag?: string; cdate?: string }> }>(`${AC_URL()}/api/3/contacts/${contactId}/contactTags`, {
    headers: AC_HEADERS(),
    timeout: 45000
  })
  const out = new Map<string, Date>()
  const contactTags = readProviderArray<{ tag?: string; cdate?: string }>(
    r.data, 'contactTags', 20_000, 'AC_CONTACT_TAGS',
  )
  chargeReadBudget(budget, contactTags.length)
...
export async function lerTagsDaAc(
  ...
  const readBudget: ReadBudget = { items: 0 }
  ...
  const tags = await todasAsTags(readBudget)
  ...
  for (const t of relevantes) {
    try {
      const contactos = await contactosDaTag(t.id, readBudget)
      ...
      report.errors.push({ contexto: `tag ${t.id} "${t.tag}"`, error: error instanceof Error ? error.message : 'erro' })
    }
  ...
  if (opcoes.comDatas !== false) {
    ...
    for (let offset = 0; offset < emails.length; offset += DB_READ_BATCH_SIZE) {
      const batchEmails = emails.slice(offset, offset + DB_READ_BATCH_SIZE)
      existentes.push(...await collectCappedCursor(ACStudentTag.find({ email: { $in: batchEmails } })
        .select('email tags.tagId tags.aplicadaEm').sort({ _id: 1 }).maxTimeMS(5_000)
        .lean().cursor({ batchSize: DB_READ_BATCH_SIZE }), DB_READ_BATCH_SIZE, 'AC_STUDENT_TAG_EXISTING') as typeof existentes)
    }
    ...
    for (const [email, reg] of porEmail) {
      ...
      try {
        const datas = await datasDasTagsDoContacto(reg.contactId, readBudget)
        for (const tag of reg.tags) tag.aplicadaEm = datas.get(tag.tagId) ?? null
        report.contactosComData += 1
      } catch (error: unknown) {
        report.errors.push({ contexto: `datas do contacto ${reg.contactId}`, error: error instanceof Error ? error.message : 'erro' })
      }
  ...
  try {
    naLista = await emailsDaLista(LISTA_OBRIGATORIA.id, readBudget)
...
export async function syncAcStudentTags(
  ...
  for (let offset = 0; offset < emails.length; offset += DB_READ_BATCH_SIZE) {
    const batchEmails = emails.slice(offset, offset + DB_READ_BATCH_SIZE)
    users.push(...await collectCappedCursor(User.find({ email: { $in: batchEmails } })
      .select('_id email').sort({ _id: 1 }).maxTimeMS(5_000)
      .lean().cursor({ batchSize: DB_READ_BATCH_SIZE }), DB_READ_BATCH_SIZE, 'AC_STUDENT_TAG_USERS') as typeof users)
  }
  ...
  if (ops.length) {
    const r = await ACStudentTag.bulkWrite(ops, { ordered: false })
...
async function esvaziarQuemPerdeuTodasAsTags(
  ...
  const comTags = await collectCappedCursor(ACStudentTag.find({ 'tags.0': { $exists: true } })
    .select('email').sort({ _id: 1 }).maxTimeMS(5_000)
    .lean().cursor({ batchSize: DB_READ_BATCH_SIZE }), MAX_MIRROR_ROWS, 'AC_STUDENT_TAG_MIRROR') as Array<{ email: string }>
  ...
  const r = await ACStudentTag.updateMany(
    { email: { $in: orfaos } },
