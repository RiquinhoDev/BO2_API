  ...
  try {
    ...
    try {
      ...
      if (config.hotmart.products.length > 0) {
        for (const product of config.hotmart.products as any[]) {
          const subdomain = product.platformData?.subdomain || product.code

          logger.info(
            `[PIPELINE] Syncing Hotmart: ${product.code} (${subdomain})`
          )
          ...
          if (hotmartData.length === 0) {
            logger.warn(
              `[PIPELINE] ⚠️ Nenhum user encontrado para ${product.code}`
            )
            continue
          }
          ...
          logger.info(
            `[PIPELINE]    ✅ ${syncResult.stats.total} users processados`
          )
        }
      } else {
      ...
      logger.info(
        `[PIPELINE] ✅ STEP 1/4 completo em ${result.steps.syncHotmart.duration}s`
      )
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err)
      const errorMsg = `Sync Hotmart: ${message}`
      ...
      logger.error('[PIPELINE] ❌ STEP 1/4 falhou:', message)
    }
    ...
    try {
      ...
      logger.info(
        `[PIPELINE] ✅ STEP 2/4 completo em ${result.steps.syncCursEduca.duration}s`
      )
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err)
      const errorMsg = `Sync CursEduca: ${message}`
      errors.push(errorMsg)
      ...
      logger.error('[PIPELINE] ❌ STEP 2/4 falhou:', message)
    }
    ...
    try {
      ...
      logger.info(
        `[PIPELINE] ✅ STEP 3/4 completo em ${result.steps.recalcEngagement.duration}s`
      )
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err)
      const errorMsg = `Recalc Engagement: ${message}`
      errors.push(errorMsg)
      ...
      logger.error('[PIPELINE] ❌ STEP 3/4 falhou:', message)
    }
    ...
    try {
      ...
      logger.info(
        `[PIPELINE] Processando ${userProducts.length} UserProducts`
      )
      ...
      const orchestrationResults =
        await tagOrchestratorV2.orchestrateMultipleUserProducts(items)
      ...
      logger.info(
        `[PIPELINE] ✅ STEP 4/4 completo em ${result.steps.evaluateTagRules.duration}s`
      )
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err)
      const errorMsg = `Tag Rules: ${message}`
      errors.push(errorMsg)
      ...
      logger.error('[PIPELINE] ❌ STEP 4/4 falhou:', message)
    }
    ...
    logger.error('═'.repeat(60))
    logger.error('[PIPELINE] Erro:', message)
