// ════════════════════════════════════════════════════════════
// 📁 src/services/sync/hotmartSync.service.ts (CORRIGIDO)
// HOTMART SYNC SERVICE
// ════════════════════════════════════════════════════════════
//
// ✅ CORREÇÃO: Integrado com hotmart.adapter.ts
// ✅ USA DADOS REAIS da API Hotmart
// ✅ Remove MOCK vazio
//
// ════════════════════════════════════════════════════════════
 ...
 * 
 * ✅ CORRIGIDO: Usa hotmart.adapter.ts para buscar dados reais
 * 
 * @param subdomain - Subdomain do produto (opcional - usa env se não fornecido)
 * @returns Resultado agregado
 */
export async function syncHotmartFull(subdomain?: string): Promise<SyncResult> {
  ...
  try {
    ...
    logger.info('[HotmartSync] 🚀 Buscando dados via hotmart.adapter...')
    ...
    if (adapterData.length === 0) {
      logger.warn('[HotmartSync] ⚠️ Adapter retornou 0 users')
      result.success = true
      return result
    }
    
    logger.info('[HotmartSync] ✅ Adapter retornou ${adapterData.length} users')
    ...
    const usersToSync: HotmartSyncData[] = adapterData.map((user) => ({
      ...
      name: user.name,
      status: "ACTIVE", // Adapter não retorna status, assumir ACTIVE
    ...
    logger.info('[HotmartSync] 📦 Convertidos ${usersToSync.length} users para sync')
    ...
    logger.info('[HotmartSync] ✅ Full sync completo', {
