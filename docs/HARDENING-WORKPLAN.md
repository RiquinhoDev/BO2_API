...
- [ ] **Nenhum ficheiro novo/tocado > ~400 linhas.** Os monstros antes citados (`clarezaCarteiraService`, `users.controller`, `classes.controller`) e `universalSyncService` já foram fisicamente apagados após extração vertical; não são dívida ativa. O inventário atual ainda tem **61 ficheiros `src/**/*.ts` acima de 400 linhas** (maiores: `cron/scheduler.ts` 1354, `syncUtilizadoresControllers/hotmart.controller.ts` 1233 e `testimonials.controller.ts` 1216); `guru.inactivation.controller.ts` foi apagado após as extrações verticais de leitura, mutações, integração externa, manutenção e discrepâncias; o router importa agora cada boundary diretamente, portanto ARCH-02 permanece aberto. `universalSync/processSyncItem.ts` (394) e `universalSync/executeUniversalSync.ts` (287) estão abaixo do alvo.
