...
- [ ] Config validada e tipada com **fail-fast** no arranque (OPS-01).
  - Estado objetivo em 2026-08-09: a fronteira/bootstrap, renewal e integrações request-driven Guru/CursEduca/Slack/student-summary estão migrados; faltam **8** reads em **4** ficheiros. A caixa só fecha a zero reads de produção fora de `src/config` e após provisioning/arranque validado no ambiente alvo.
