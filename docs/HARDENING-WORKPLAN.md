...
- [x] **SCALE-01 inventariado:** os 40 reads classificados estão code-complete; a migração mais ampla das restantes listagens HTTP/scans continua fora deste lote.
  ...
  - **SCALE-03 B+C (2026-09-05, Round 2; operacional aberto):** inventário canónico de código 24 = 20 complete (16 changed + 4 already-compliant) + 4 pending. As seis decisões agora fechadas são explicitamente `constrained-sequential`: student movement, activity snapshot writes, achievement evaluation persistence, Guru discrepancy compensation, Guru expired-trial writes e testimonial tag provider sync; os testes N=1/10/100 provam peak 1, ordem, compensação, falha parcial e contagem completa de tentativas/falhas. `lastSyncedAt` só avança após sucesso completo do provider e não avança em no-op recente ou falha de persistência. Activity snapshots contam cada user tentado em `totalProcessed`, separam writes bem-sucedidos em `snapshotsCreated` e devolvem erros por user/plataforma em ordem. Achievement batch mantém avaliação→persistência por user, conta `processed` antes da tentativa, `evaluated` só após persistência e continua a ordem após falhas. Permanecem 4 decisões com risco semântico de writes parciais, provider ou batching ainda não provado. O gate operacional permanece `pending`: nenhum target Mongo explicitamente autorizado, não-produtivo e read-only foi carregado para `executionStats` ou probes 1/10/50.
