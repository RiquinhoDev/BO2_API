...
- `POST /api/activecampaign/product-tags/apply`, `remove` e `products/:productId/tags/sync` now use durable receipts keyed by `(operation, target identity, X-Request-ID)` and a partial unique running receipt per target. The existing `UserProduct` claim remains the local ownership fence; `sync` applies the receipt per enrollment and keeps the **200** query cap before provider I/O. A replayed request returns its stored result without repeating provider or local mutation; a concurrent target returns `409`.
...
- Fresh offline proof for this product-tag slice: focused gate passed **6 suites / 37 tests**; the full backend gate passed **481/483 suites** and **3075/3087 tests** (**2 suites / 12 tests skipped**). Types, build, lint, route catalog, response catalog (**409 decisions; 213 Front calls; 188 consumers**), SCALE (**40/0, 11/0, 24/0; 368 Mongoose sites**) and diff-check also passed. No provider, production datastore, Railway, domain or network was contacted. OPS-02 hardening debt moves from **16 to 13** decisions (**6 mixed, 7 provider, 10 bulk**).
