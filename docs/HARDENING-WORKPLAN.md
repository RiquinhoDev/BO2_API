...
- Contradições ainda abertas: o error handler central convive com **302** respostas 500 ad hoc; restam **45** leituras raw-env, **671** suppressions `no-explicit-any`, a política transversal de idempotência/caps e o inventário/migração de listagens HTTP não canónicas + scans `find({})`. Distributed limiter/429/CSP, JWT dedicado, CORS explícito e debug local estão fechados no código; provisioning/deploy/observação continuam separados.
...
- [ ] Config validada e tipada com **fail-fast** no arranque (OPS-01).
  - Estado objetivo em 2026-08-09: a fronteira/bootstrap, renewal e integrações request-driven Guru/CursEduca/Slack/student-summary estão migrados; faltam **45** reads em **17** ficheiros. A caixa só fecha a zero reads de produção fora de `src/config` e após provisioning/arranque validado no ambiente alvo.
