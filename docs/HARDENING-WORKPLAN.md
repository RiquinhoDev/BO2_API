...
- [x] **ARCH-02/regra #9 — auto-inativação Hotmart morta removida (2026-08-08):** o executor estava permanentemente atrás de `const false`; o collector só alimentava esse ramo e não tinha saída operacional. A deteção de expiração e o log de revisão manual foram preservados, mas os writes inalcançáveis sobre User/UserProduct/Class/UserHistory, o collector e os testes órfãos foram apagados. `executeUniversalSync.ts` caiu **507→287 linhas** e `processSyncItem.ts` **401→394**; no-console do executor **25→13**. Mutação `false→true` deu RED ao inativar o aluno expirado; rede Hotmart/shared 2 suites/15 testes e gate maior: lint/types/build 0 e 226/226 suites, 1318/1318 testes.
  **Follow-up ambiental para Claude:** o helper `apply_patch` do sandbox falhou repetidamente ao aplicar ACLs de leitura; a edição foi revista e aplicada por fallback local. `npx prettier --check` ficou em timeout porque este repo não tem binário Prettier local; foi terminado, não instalou dependências e não alterou lockfiles. Repetir apenas num ambiente com a ferramenta já disponível; os gates oficiais lint/types/Jest/build ficaram verdes.

- [x] **ARCH-02/SEC-10 — Guru inactivation dissolvido (2026-08-09):** o barrel residual `guru.inactivation.controller.ts` (286 linhas) foi apagado; as 14 rotas importam agora diretamente os boundaries de leitura, mutação local, integração externa, manutenção e discrepâncias. `mark-discrepancies` passou a serviço/repositório/clientes injetáveis + boundary strict, preserva o envelope e acrescenta o `noUserProduct` exigido pelo Front. Os últimos **2→0** HTTP 500 locais deste domínio foram enviados para SEC-10; o baseline global medido caiu **237→235** e a suppression `no-console` do ficheiro morto caiu **21→0**. Dois bugs de persistência strict foram corrigidos no schema (`markedFromComparison`, `revertReason`). RED/GREEN cobre decisões, adapter Mongo, contrato, campos hostis e metadata; zero integrações reais.

- [x] **ARCH-02 — boundary de cursos ActiveCampaign extraído (2026-08-09):** os dashboards e previews Clareza/OGI saíram de `acTags/activecampaign.controller.ts` para `activeCampaignCourse.controller.ts`, mantendo as duas famílias de rotas e o dry-run real (`evaluateAllUsersOfProduct(..., true)`). O monólito caiu **1413→1067 linhas**; o novo boundary tem **355 linhas**. Characterization com mutações RED protege lookup, zero envelopes, agregação, classificação/reconciliação de tags e falhas; **14→0** consoles migrados para o logger canónico sem novas suppressions. A hipótese de shadowing das rotas GET específicas foi descartada (`/:id` não corresponde a dois segmentos); os placeholders aleatórios continuam fora deste bloco e exigem decisão própria.
- [x] **ARCH-02 — controller residual ActiveCampaign dissolvido** (2026-08-09): `acTags/activecampaign.controller.ts` foi fisicamente apagado. Operações manuais, CRUD legacy de regras e tags V2 passaram para owners diretos com **225/116/326 linhas**; nenhuma rota depende de barrel compatível. A sequência completa do antigo monólito caiu **1413→1067→656→0 linhas**. Characterization protege contadores/log do `test-cron`, ordenação last-20, envelopes legacy, persistência UserProduct e partial sync; mutações reais deram RED ao zerar `actionsExecuted` e ao remover `UserProduct.save()`. O efeito `findOrCreateContact` do remove foi preservado, eliminando apenas a variável não usada. Dívidas explícitas: `test-cron` continua mutativo sem gate próprio e os aliases legacy de tag rules permanecem por compatibilidade até decisão/observação; nenhuma integração real foi executada.
- [x] **ARCH-02 — reporting de histórico ActiveCampaign extraído** (2026-08-09): listagem, estatísticas e formatação de motivo saíram de `acTags/activecampaign.controller.ts` para três módulos coesos com **197/201/47 linhas**. As rotas `/api/activecampaign/*` e o alias app-level `/api/communication-history` importam agora o boundary proprietário diretamente. O monólito caiu **1067→656 linhas**; nenhuma unidade nova ultrapassa ~400. Characterization preserva precedência de filtros, populate/sort/skip/limit, fallback `sentAt`, pipeline legado por `timestamp`, envelopes vazios e erros públicos estáveis; mutações reais deram RED para `sentAt→createdAt` e `timestamp→createdAt`. As divergências `action` ignorado e `createdAt/tagApplied` vs. `timestamp/action/tagName` ficam registadas como dívida de contrato, não alteradas por esta extração.
...
- [x] **SCALE-01 inventariado:** os 40 reads classificados estão code-complete; a migração mais ampla das restantes listagens HTTP/scans continua fora deste lote.
  ...
  - **SCALE-03 B+C (2026-09-06, Round 2; operacional aberto):** inventário canónico de código 24 = 24 complete (20 changed + 4 already-compliant) + 0 pending. As dez decisões agora fechadas são explicitamente `constrained-sequential`: student movement, activity snapshot writes, achievement evaluation persistence, Guru discrepancy compensation, Guru expired-trial writes, testimonial tag provider sync, weekly tag snapshot writes, native tag protection capture, Guru cross-reference actions e product-sales product traversal; os testes N=1/10/100 provam peak 1, ordem, compensação, falha parcial, reparação e contagem completa de tentativas/falhas. `lastSyncedAt` só avança após sucesso completo do provider e não avança em no-op recente ou falha de persistência. Activity snapshots contam cada user tentado em `totalProcessed`, separam writes bem-sucedidos em `snapshotsCreated` e devolvem erros por user/plataforma em ordem. Achievement batch mantém avaliação→persistência por user, conta `processed` antes da tentativa, `evaluated` só após persistência e continua a ordem após falhas. Weekly snapshots usam identidade única email normalizado + semana/ano, distinguem insert de replay, contabilizam falhas parciais e reparam notificações/detalhes incompletos sem duplicar rows; details são escritos sequencialmente. `notificationsCreated` só conta uma notificação agrupada cuja row, details e link terminam com sucesso; uma row criada antes de uma falha parcial fica reparável por replay e não é contada como concluída. Native tag protection normaliza email, usa leitura ActiveCampaign strict (erro não vira `[]`), valida `batchSize` inteiro positivo, preserva ordem provider→snapshot, regista remoções totais e converte duplicate-key de identidade única em replay. Guru cross-reference mantém writes sequenciais, conta cada tentativa de provider no limite 20, regista falhas de leitura sem declarar sucesso, normaliza/deduplica uma vez os emails para as queries, threshold e reconciliação stale, e mantém input scoped que canoniza vazio em `$in: []` sem alargar o universo. Product-sales percorre produtos em ordem, devolve contadores/errors por produto, limpa stats de produtos sem UserProducts e só conta upsert com documento persistido; falha de um produto não interrompe os seguintes e retry reprocessa o mesmo `productId`. O gate operacional permanece `pending`: nenhum target Mongo explicitamente autorizado, não-produtivo e read-only foi carregado para `executionStats` ou probes 1/10/50.
...
- [ ] Config validada e tipada com **fail-fast** no arranque (OPS-01).
  - Estado em 2026-08-09: o gate de código está fechado em **0 reads / 0 ficheiros runtime** fora das composition roots explícitas (`appConfig`, config de BD de teste e dois CLIs de manutenção com ambiente injetado). A caixa permanece operacionalmente aberta apenas até provisioning e arranque serem validados no ambiente alvo.
...
#### Atualização verificável — 2026-09-06 — strict ActiveCampaign read boundary — BO2_API `remake`

- Correção crítica: o contrato legacy `getContactTagsByEmail` continua fail-soft para callers existentes, mas native tag protection e weekly tag monitoring usam `getContactTagsByEmailStrict`, que distingue contacto ausente/sem tags (`[]` autoritativo) de falha de leitura (erro propagado). Assim, erro provider não cria snapshot vazio nem history `REMOVED`.
- RED/GREEN na fronteira real: erro genérico de leitura que o método legacy engolia passa no strict; native falha com `success:false/errors++` sem mutar snapshot/history; weekly mantém o mesmo fail-closed. Foco afetado: **3 suites / 38 testes passed**.
- `types:check`, `lint`, `scalability:reads:check` e `git diff --check`: **PASS**. Sem provider, Mongo real, API externa, rede, Railway, domínio, BD temporária ou promoção.

#### Atualização verificável — 2026-09-06 — native-tags follow-up — BO2_API `remake`

- O follow-up native-tags fecha a decisão de código SCALE-03: inventário canónico **22 complete / 2 pending** (18 changed + 4 already-compliant). Captura mantém provider → snapshot/history por email, continua após falhas parciais e faz replay seguro de saves falhados e duplicate-key concorrente.
- RED/GREEN focado: **2 suites / 21 testes passed**, com **N=1/10/100**, peak **1**, remoção total, history, retry, normalização de email e boundaries `batchSize` inválidos. Índice único de email está no schema; migração/limpeza de duplicados existentes continua operacional pending.
- `types:check`, `lint`, `scalability:reads:check` e `git diff --check`: **PASS**. Sem provider, Mongo real, API externa, rede, Railway, domínio, BD temporária ou promoção.

#### Atualização verificável — 2026-09-06 — Guru cross-reference follow-up — BO2_API `remake`

- O cross-reference fecha a decisão SCALE-03 com uma correção pequena e sem paralelização: budget CursEduca passa a contar tentativas antes do provider, falhas entram em `result.errors` e a reconciliação stale normaliza `syncedEmails` antes da comparação com emails da BD.
- RED/GREEN focado: **1 suite / 13 testes passed**, com N=1/10/100, peak 1, ordem de writes, falhas parciais, throttle de 300 ms, teto de 20 tentativas mesmo quando o provider falha e identidade stale case/whitespace-safe. Ratchet de inventário: **SCALE-03 23 complete / 1 pending** (19 changed + 4 already-compliant).
- Sem provider, Mongo real, API externa, rede, Railway, domínio, BD temporária ou promoção. Fecho operacional continua pendente.

#### Atualização verificável — 2026-09-06 — Guru cross-reference identity follow-up — BO2_API `remake`

- Follow-up fecha a fronteira de identidade: `syncedEmails` é normalizado, filtrado e deduplicado uma vez; a mesma lista canónica alimenta `$in`, `$nin`, `minSyncSize`, logging e membership stale. Lista bruta com 400 variantes do mesmo email já não dispara reconciliação massiva.
- RED/GREEN focado: **1 suite / 14 testes passed**, incluindo query canonical case/space-safe, duplicados pesados e bloqueio do stale threshold por cardinalidade real. Sem mudança no budget provider, ordem ou throttle.
- Sem provider, Mongo real, API externa, rede, Railway, domínio, BD temporária ou promoção. Fecho operacional continua pendente.

#### Atualização verificável — 2026-09-06 — Guru cross-reference scoped-input follow-up — BO2_API `remake`

- Correção de boundary: input fornecido e não vazio que canoniza para zero emails mantém filtro explícito `$in: []`; não cai no modo sem filtro e não pode processar a população inteira. `undefined` e `[]` mantêm o comportamento legacy sem filtro; passes missed/stale continuam reservados a lista canónica não vazia.
- RED/GREEN focado: **1 suite / 15 testes passed**, com blank-only input, zero `UserProduct` reads/writes e query `$in: []`. Sem mudança no budget provider, ordem ou throttle.
- Sem provider, Mongo real, API externa, rede, Railway, domínio, BD temporária ou promoção. Fecho operacional continua pendente.

#### Atualização verificável — 2026-09-06 — product-sales product-loop follow-up — BO2_API `remake`

- O traversal externo de Product Sales Stats passa a isolar cada produto: mantém reads/upsert sequenciais, devolve `productsProcessed`/`productsSucceeded`/`errors`, trata resultado `null` do upsert como falha e continua para produtos seguintes. Produto ativo sem UserProducts agora sobrescreve stats antigas com agregados vazios.
- RED/GREEN focado: **1 suite / 7 testes passed**, com N=1/10/100, peak 1, ordem de produtos, um upsert por produto, zero-dataset, falhas isoladas de leitura/escrita, retry e null-upsert. Inventário SCALE-03: **24 complete / 0 pending** (20 changed + 4 already-compliant).
- Sem provider, Mongo real, API externa, rede, Railway, domínio, BD temporária ou promoção. Fecho operacional continua pendente.

#### Atualização verificável — 2026-09-06 — SCALE-03 follow-up — BO2_API `remake`

- O checkpoint de `2026-09-05` abaixo permanece histórico. Neste follow-up, o inventário canónico SCALE-03 está em **21 complete / 3 pending** (17 changed + 4 already-compliant).
- RED/GREEN focado: **3 suites / 23 testes passed**; cobre remoção total de tags nativas, reparação por replay, preservação de `detectedAt` e métrica `notificationsCreated` apenas após sincronização completa. `types:check`, `scalability:reads:check`, `lint` e `git diff --check`: **PASS**.
- Sem provider, Mongo real, API externa, rede, Railway, domínio, BD temporária ou promoção. Fecho operacional continua pendente.
...
código verificado antes deste registo foi `f0735fc2` (`chore(scalability): close constrained write decisions`).
...
- Gates frescos: `types:check`, `build`, `lint`, `routes:catalog:check` (**409** identidades),
  ...
  `scalability:reads:check` (**SCALE-01 40 complete / 0 pending; SCALE-02 11 / 0; SCALE-03 20 / 4
...
- Ainda pendente: Docker disponível/validado, Railway, domínios de teste, BD temporária isolada, browser/live
  QA, deploy/promoção, idempotência provider OPS-02 e os **4** itens SCALE-03 pendentes.
...
### [x] SCALE-03 — weekly tag snapshot convergence (2026-09-06)

- Weekly tag monitoring remains provider → user → snapshot upsert → previous comparison → change history → grouped notification → cleanup, with one write in flight. `snapshotsCreated` counts only new weekly identities; replay reports zero new snapshots and zero new notifications.
- Added unique identities for `(email, weekNumber, year)` and `(notificationId, email)`. Notification claiming is protected by the existing unique tag/type/week/year index; details use deterministic upserts with the claimed notification ID, so duplicate workers do not leave null-linked or orphan details. Partial detail/link failures remain retryable on replay.
- Fresh focused RED/GREEN proof: **3 suites / 23 tests passed**; weekly and notification harnesses cover **N=1/10/100**, peak **1**, ordered partial failures, total native-tag removals, same-week replay, duplicate claims, partial detail repair, link repair, detectedAt preservation and detail-upsert order. TypeScript ratchet passed; no provider, real Mongo, external API or network was contacted.
- SCALE-03 moves from **20/4** to **21/3** code decisions (**17 changed + 4 already-compliant**). Operational closure remains pending: index migration/duplicate cleanup, isolated database, Railway/domains, live/browser QA and promotion are not claimed here. `notificationsCreated` remains a completed-synchronization metric; partial row creation is intentionally repaired on replay instead of counted as complete.

### [x] SCALE-03 — native tag protection capture (2026-09-06)

- Native tag capture now keeps provider reads and per-email snapshot/history writes strictly sequential, including total provider-side removals and ordered partial-error accounting.
- Snapshot identity is normalized (`trim` + lowercase) and unique by email. A duplicate-key race is treated as a replay of the winner; existing rows are reconciled instead of returning a false success or creating a duplicate.
...
### [x] OPS-02 — ActiveCampaign execution and product-tag endpoints (2026-09-06)
...
- Both live endpoints cap the active `UserProduct` execution universe at `MAX_BULK_OPERATION_ITEMS` (**200**), reject the cap before provider evaluation, and use durable Mongo receipts keyed by `(operation, X-Request-ID)` plus a partial unique running-operation lease. A heartbeat renews the owner during long runs; the ActiveCampaign transport refuses a new provider attempt after heartbeat ownership is lost. Completed receipts survive later requests: the same `X-Request-ID` replays its canonical success response; a concurrent live request receives `409`; an expired lease is fenced before recovery.
...
- `POST /api/activecampaign/product-tags/apply`, `remove` e `products/:productId/tags/sync` now use durable receipts keyed by `(operation, target identity, X-Request-ID)` and a partial unique active (`running`/`indeterminate`) receipt per target. The existing `UserProduct` claim remains the local ownership fence; `sync` applies the receipt per enrollment and keeps the **200** query cap before provider I/O. A replayed request returns its stored result without repeating provider or local mutation; a concurrent target returns `409`; an `indeterminate` receipt blocks every new request for that target until explicit reconciliation.
- The provider contracts remain explicitly narrower than route idempotency: contact creation and tag-link creation still use provider-side read-then-create sequences, and remove treats an absent contact/tag/link as converged but cannot make a repeated provider `DELETE` atomic. The route receipt prevents duplicate route replay, not every provider race. Provider success followed by local/receipt persistence failure is recorded as `indeterminate` and is not claimed exactly-once.
- A provider success followed by a receipt-completion database failure remains an at-least-once / indeterminate window; this slice does not claim exactly-once provider semantics.
...
- Genuine contact absence/empty tags remains authoritative empty; an error cannot be persisted as `nativeTags=[]` or a false `REMOVED` history event. Focused affected proof: **3 suites / 38 tests passed**.

### [x] OPS-02 — Discord renewal message receipts (2026-09-06)

- `POST /api/discord-renewal/messages/send`, `POST /api/discord-renewal/scheduled/:key/test` e `POST /api/discord-renewal/scheduled/run` agora propagam o `X-Request-ID` real (ou correlation fallback) para receipts Mongo duráveis. O mesmo pedido devolve o resultado canónico; payload, target ou actor diferente com o mesmo ID é rejeitado atomically; concorrência recebe `409` e lease expirado torna-se `indeterminate` sem reabrir provider.
- O envio fica protegido por lease/heartbeat e fence imediatamente antes da chamada ao bot. Provider success seguido de falha em `DiscordMessageLog`, settlement do receipt ou save da regra fica `indeterminate`/`503`, sem nova tentativa automática; exactly-once remoto continua fora do alcance do BO2_API.
- `scheduled/run` aceita no máximo **50** regras (consulta sentinel 51) e rejeita regras existentes + defaults em falta acima do limite antes de seed/provider; `dryRun` faz apenas leituras e planeamento, sem provider, receipt, seed ou `save`. O cron interno sem header usa receipt estável por regra+mês, além do receipt do run HTTP. Falhas de rota passam pela fronteira central `HttpError`.
- Fresh offline proof: focused **7 suites / 57 tests passed** nesta slice; integração Mongo real **14/14** com índices inicializados e `--detectOpenHandles`; TypeScript, route catalog, response catalog (**409 decisions; 213 Front calls; 188 consumers**), SCALE (**40/0, 11/0, 24/0; 368 Mongoose sites**) e diff-check passaram. Full Jest fresco com root Front explícito passou **485 suites / 3.137 testes**, com **2 suites / 12 testes skipped** previstos.
- OPS-02 hardening debt baixa para **8** decisões (**6 mixed, 2 provider, 8 bulk**). Não foram contactados Discord real, provider, Mongo de produção, Railway, domínio ou rede; provisioning isolado, observação externa, deploy Front/BO e promoção estável continuam pendentes.

### [x] OPS-02 — Clareza/FMP refresh receipts (2026-09-06)

- Os seis refreshes `POST /api/clareza/refresh`, `/top10/refresh`, `/raiox/refresh`, `/carteira/refresh`, `/earnings/refresh` e `/comparador/refresh` usam um receipt Mongo durável por `(operation, X-Request-ID)`, com fence parcial única por `(operation, identity, fingerprint)` em `running`/`indeterminate`. O mesmo pedido reproduz o resultado canónico; payload/símbolos diferentes com o mesmo ID são rejeitados atomicamente; concorrência recebe `409`; lease expirado ou perda de posse fica `indeterminate` sem reabrir FMP.
- Os caminhos reais sinalizam explicitamente `providerStarted`, `providerSucceeded` e `localMutationStarted`; a fence local começa na primeira escrita. Falha FMP antes dessa fence fica retryable; falha depois de Redis/local, ou settlement do receipt depois de sucesso, fica `indeterminate`/`503`. A ordem existente mantém-se — Raio-X escreve sector/SPY e caches por ticker enquanto ainda lê FMP — e os snapshots Mongo continuam best-effort. Como FMP é read-only, não há mutação remota exactly-once a reclamar; a incerteza é local/receipt.
- Comparator normaliza, deduplica e preserva a ordem dos símbolos (`AAPL,MSFT` pode diferir de `MSFT,AAPL`). Autorização Clareza continua antes de claim/replay; `killSwitch`/`dryRun` permanecem `not-applicable` por FMP read-only. O controller usa `X-Request-ID` real ou correlation fallback e mantém 403/400/sucesso.
- Fresh offline proof: receipt MongoMemoryServer **1 suite / 12 testes passed** (`--detectOpenHandles`), controllers/policy/services focados **5 suites / 59 testes passed**; full backend **487 suites / 3162 testes passed**, com **2 suites / 12 testes skipped** previstos. TypeScript, lint, build, route catalog (**409 identities**), response catalog (**409 decisions; 213 Front calls; 188 consumers**), SCALE (**40/0, 11/0, 24/0; 368 Mongoose sites**) e `git diff --check` passaram. Sem FMP/Redis/Mongo de produção, Railway, domínios ou rede; provisioning isolado, observação externa, deploy Front/BO e promoção estável continuam pendentes.
- OPS-02 hardening debt baixa para **2** decisões (**0 mixed, 2 provider, 2 bulk**).
