...
- Fresh focused RED/GREEN proof: **3 suites / 19 tests passed**; weekly and notification harnesses cover **N=1/10/100**, peak **1**, ordered partial failures, same-week replay, duplicate claims, partial detail repair, link repair and detail-upsert order. TypeScript ratchet passed; no provider, real Mongo, external API or network was contacted.
- SCALE-03 moves from **20/4** to **21/3** code decisions (**17 changed + 4 already-compliant**). Operational closure remains pending: index migration/duplicate cleanup, isolated database, Railway/domains, live/browser QA and promotion are not claimed here.
