...
>
> **Runbook de activação**: 1) deploy BO2+Front; 2) em domínio Railway + BD de teste isolados, pré-visualizar cada regra e executar "Testar sem menções"; 3) validar `dryRun` do run (zero provider e zero mutação local); 4) env `DISCORD_SCHEDULED_MESSAGES_ENABLED=true` no BO_v2; 5) ligar o cron `DiscordScheduledMessages` na UI de CRON Jobs; 6) ligar as 2 regras na secção Agendadas. O run aceita no máximo 50 regras e rejeita também o caso em que os defaults em falta fariam ultrapassar esse limite, sempre antes de seed/provider; receipts por `X-Request-ID` + fence por regra/mês evitam replay concorrente. Replay canónico, payload/actor diferente, lease expirado e falha local pós-provider ficam bloqueados/`indeterminate` para reconciliação. O provider continua at-least-once; não há exactly-once remoto. Emergência: desligar master/cron/regra — qualquer um trava.
...
- **Cron `DiscordScheduledMessages`** (novo, proposta 10:00 Europe/Lisbon, **nasce DESLIGADO**, seed create-only): aceita no máximo 50 regras (a consulta usa um sentinel de 51) e rejeita regras existentes + defaults em falta acima desse limite antes de qualquer seed/provider; para cada regra `enabled` com `dayOfMonth === hoje` e `lastSentMonth !== mêsActual`: renderiza o template com o mês alvo (e dataFim = dia 15 de M+1 para o texto), envia via `sendDiscordMessage()` **existente** (mesmas allowlists dos 12 cargos R.* + canais, mesmo `DiscordMessageLog`) e grava `lastSentMonth` apenas depois do provider + auditoria local sob a fence durável.
- **Receipts de execução**: `DiscordMessageExecutionReceipt` usa `(operation, requestId)` único, fence activa por identidade, lease/heartbeat e estados `completed`, `failed` e `indeterminate`. `messages/send`, `scheduled/:key/test` e `scheduled/run` recebem `X-Request-ID`; replays devolvem o resultado canónico sem releitura/preflight mutável. O scheduler interno sem header usa a identidade estável `regra + mês`.
...
- **Idempotência**: a fence atómica do receipt garante no máx. 1 tentativa proprietária por regra/mês em re-runs/concorrência; `lastSentMonth` é a projeção de estado. Provider success seguido de falha em `DiscordMessageLog` ou no save da regra fica `indeterminate`, não retryable.
...
- **Seeds default (desligadas)**: `lembrete-dia-8` (dia 8) e `ultimo-aviso-dia-15` (dia 15), apontando aos templates de renovação existentes.
