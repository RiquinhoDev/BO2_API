...
O risco atual é, porém, alto. O achado mais grave não é apenas “falta de autorização por papel”: quase toda a API está montada sem autenticação. Rotas que inativam alunos, executam sincronizações, escrevem/removem tags, executam CRONs e apagam snapshots estão acessíveis sem o middleware existente. O histórico do build tinha um falso verde antes do TOOL-01: foram medidos **568 erros TypeScript em 87 ficheiros**, mas `tsc || exit 0` devolvia sucesso; no HEAD actual, o gate aplica `strict:true`, `noEmitOnError:true`, compilação directa sem emissão e build emissor estrito.
...
- O commit `3a5f5ea` declarou Jest/ts-jest, `mongodb-memory-server` e Supertest e sincronizou `package-lock.json` e `yarn.lock`; essa era a autoridade dupla observada naquele momento.
- **TOOL-03 resolvido em 2026-08-03:** `package.json` fixa `packageManager` em `npm@11.9.0`; `Dockerfile` e `nixpacks.toml` executam npm (`npm ci`, `npm run build`, `npm start`); `package-lock.json` é o único lockfile sobrevivente e `yarn.lock` foi removido. Alterações futuras de dependências devem atualizar e validar apenas o lockfile npm, sem alterar versões por este achado.
...
| ID | Severidade | Esforço | Achado e evidência | Recomendação |
|---|---|---:|---|---|
| TOOL-01 | **Alta** | L | **Fechado em 2026-08-03 (Task 1 `8ee1c7c` + Task 2).** O `tsconfig.json` aplica `strict:true` e `noEmitOnError:true`; `package.json#scripts.types:check` executa directamente `tsc --noEmit --pretty false`; `build` executa o `tsc` emissor. No HEAD actual foram removidos o ratchet, a baseline, o teste do ratchet e qualquer bypass `tsc || exit 0`; o contrato de tooling impede regressão. Gates offline actuais: lint 0, TypeScript 0, Jest com `MONGOMS_RUNTIME_DOWNLOAD=false` — 159 suites passed / 1 skipped e 801 testes passed / 2 skipped, build 0. | Manter `strict`, `noEmitOnError`, a compilação sem emissão e o build emissor como autoridades. Alterações futuras ao gate devem preservar estes contratos e repetir os gates offline (incluindo `git diff --check`). |
| TOOL-02 | **Alta** | M | **ESLint ratcheted entregue na F1.10.** A flat config aplica regras recommended de JavaScript/TypeScript, regras explícitas de segurança e `no-console` global. A baseline nativa contém 2904 suppressions por ficheiro+regra: 2681 `no-console`, 115 `no-unused-vars` e 108 das restantes regras. Não existe comando para aumentar a baseline; suppressions obsoletas fazem o gate falhar até `lint:baseline:prune`. | Manter `--max-warnings=0`, podar a baseline à medida que a dívida desce e nunca usar `--pass-on-unpruned-suppressions`. Continuar a reduzir `no-explicit-any` sob `strict:true`; TOOL-02 permanece aberto até a baseline estar a zero. |
| TOOL-03 | **Alta** | S/M | **Resolvido em 2026-08-03.** `Dockerfile`, `nixpacks.toml` e `package.json` selecionam npm; `package-lock.json` é o lockfile autoritativo único e `yarn.lock` foi removido. A resolução de dependências permaneceu inalterada neste commit. | Manter npm como autoridade única: usar `npm ci` nos ambientes de build e atualizar apenas `package-lock.json` quando houver uma alteração de dependências aprovada. |
...
| ARCH-04 | **Média** | M/L | **Observação preparada, remoção pendente.** O catálogo cobre 455 rotas e as 18 montagens duplicadas de `cron-tags` estão marcadas `deprecated`, com razão escrita. A instrumentação central regista método, path template redigido, autenticação e distingue os mounts `api`/`app`; as candidatas emitem `Deprecation: true`. Nenhuma rota foi removida. | Fazer deploy e definir uma janela de observação de tráfego real. Só depois decidir a remoção; ausência de consumidor no Front não prova que outros domínios estejam mortos. |
