...
| ID | Severidade | Esforço | Achado e evidência | Recomendação |
|---|---|---:|---|---|
| TOOL-01 | **Alta** | L | **Build falso verde e dívida de tipos grande.** `package.json` mantém temporariamente `tsc || exit 0`; `tsconfig.json` tem `noEmitOnError:false`, `strict:false`, `skipLibCheck:true`. Remover `@types/mongoose` e `@types/express-validator` não alterou o baseline de 568 erros/87 ficheiros/391 TS2349. A causa real eram 22 models com guards `mongoose.models.X || mongoose.model<I>()`: a anotação explícita de `mongoose.Model<I>` reduziu para 194 erros/45 ficheiros e eliminou os TS2349. | Manter o baseline ratcheted por diretório e a allowlist dos ficheiros sujos, baixando ambos até zero. Adicionar ESLint incremental; só então tornar `tsc --noEmit` gate final, ativar `noEmitOnError:true` e remover o bypass. `strict` entra em ondas posteriores. |
