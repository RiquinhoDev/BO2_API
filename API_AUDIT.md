...
| ID | Severidade | Esforço | Achado e evidência | Recomendação |
|---|---|---:|---|---|
| TOOL-01 | **Alta** | L | **Build falso verde e dívida de tipos grande.** `package.json:48` usa `tsc || exit 0`; `tsconfig.json:73,88,111` tem `noEmitOnError:false`, `strict:false`, `skipLibCheck:true`. `tsc --noEmit` mediu 568 erros/87 ficheiros; 391 são TS2349, em grande parte coerentes com `@types/mongoose@5.11.97` sobre `mongoose@8.21.0` (`package.json:20,36`). | Não remover apenas o bypass e partir deploy. Primeiro remover tipos stub/obsoletos (`@types/mongoose`, `@types/express-validator`, `@types/xlsx`), criar baseline ratcheted por módulo e baixar a zero. Depois tornar `tsc --noEmit` gate e `noEmitOnError:true`; ativar strict em ondas. |
| TOOL-02 | **Alta** | M | **Lint inexistente.** `package.json:49` chama ESLint, mas não há config nem `eslint`/`@typescript-eslint` nas devDependencies. | Adicionar flat config mínima TypeScript; começar por regras de segurança/bugs e `--max-warnings=0` apenas no scope limpo. Expandir por módulos sem autofix massivo. |
...
4. **Tooling ratcheted:** remover `@types` errados, medir/corrigir TypeScript por módulo, ESLint incremental; só então eliminar `|| exit 0`.
